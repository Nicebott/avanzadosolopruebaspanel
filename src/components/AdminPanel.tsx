  ...
  if (loading) {
    return (
      <div className={`flex flex-col items-center justify-center h-64 ${
        ...
        <span className="ml-2 text-sm sm:text-base mt-2">Verificando permisos...</span>
      </div>
    );
  }
  ...
  if (!isAdmin) {
    return (
      <div className={`text-center py-12 px-4 ${
        ...
        <h3 className="text-lg sm:text-xl font-medium mb-2">Acceso Denegado</h3>
        <p className="text-sm sm:text-base">No tienes permisos de administrador para acceder a esta sección.</p>
      </div>
    );
  }
      ...
      >
        <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-3 mb-2 sm:mb-3">
          <div className="flex items-center gap-2 sm:gap-3">
            <Shield className={`w-6 h-6 sm:w-7 sm:h-7 md:w-8 md:h-8 ${darkMode ? 'text-yellow-400' : 'text-yellow-500'}`} />
            <h2 className={`text-xl sm:text-2xl md:text-3xl font-bold ${darkMode ? 'text-white' : 'text-gray-900'}`}>
              Panel de Administración
            </h2>
          </div>
          {isSuperAdmin && (
            <span className="px-2 sm:px-3 py-1 text-xs sm:text-sm bg-gradient-to-r from-yellow-500 to-orange-500 text-white rounded-full font-bold shadow-lg w-fit">
              SUPERADMIN
            </span>
          )}
        </div>
        <p className={`text-xs sm:text-sm md:text-base ${darkMode ? 'text-gray-400' : 'text-gray-600'}`}>
      ...
      >
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
          ...
          <div className="flex items-center gap-1.5 sm:gap-2 w-full sm:w-auto overflow-hidden">
            <code className={`px-2 sm:px-3 py-1.5 sm:py-2 rounded text-[10px] sm:text-xs md:text-sm font-mono truncate flex-1 sm:flex-none ${
            ...
            >
              {copiedUid === auth.currentUser?.uid ? <Check size={14} className="sm:w-4 sm:h-4" /> : <Copy size={14} className="sm:w-4 sm:h-4" />}
            </button>
          </div>
        </div>
      </motion.div>
              ...
              >
                <div className="flex-1 min-w-0 mr-2">
                  <div className="flex items-center gap-1 sm:gap-2 mb-0.5">
                    <p className={`font-medium text-[10px] sm:text-xs md:text-sm truncate ${darkMode ? 'text-white' : 'text-gray-900'}`}>
                      {admin.id}
                    </p>
                    <button
                    ...
                    >
                      {copiedUid === admin.id ? <Check className="w-3 h-3 sm:w-3.5 sm:h-3.5" /> : <Copy className="w-3 h-3 sm:w-3.5 sm:h-3.5" />}
                    </button>
                  </div>
                  <p className={`text-[10px] sm:text-xs md:text-sm ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>
                    Agregado: {new Date(admin.addedAt).toLocaleDateString('es-DO', {
                      day: '2-digit',
                      month: 'short',
                      year: 'numeric'
                    })}
                  ...
                  {admin.id === auth.currentUser?.uid && (
                    <span className="inline-block px-1.5 sm:px-2 py-0.5 sm:py-1 text-[9px] sm:text-[10px] md:text-xs bg-blue-500 text-white rounded-full mt-1">
                      Tú
                    </span>
                  )}
                </div>
